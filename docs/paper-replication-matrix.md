...
| Taxonomy expansion alternates best-of-N proposals, critic refinement, and optional per-level planning (Sec. 2.1, App. B.4). | Approximated | Opt-in NIM taxonomy expansion supports configurable best-of-N proposal pools (`SIMULA_TAXONOMY_PROPOSAL_COUNT`) and a strict refinement request (`SIMULA_TAXONOMY_REFINEMENT`), with resolved settings persisted in `generation_policy`; per-level planning and paper-scale selection evidence remain incomplete. |
| Agentic synthesis samples compatible node mixes, generates multiple scenarios, complexifies a fraction, then performs generation and critic refinement (Sec. 2.2, App. C). | Approximated | Stages and lineage exist; opt-in NIM providers generate local scenarios and complexified text with strict ordered responses. `local_diversification_config.node_mix_size` (or `SIMULA_LOCAL_NODE_MIX_SIZE`) now batches same-depth taxonomy nodes and persists the full mix in lineage; learned compatibility sampling, regeneration refinement, and paper-scale live evidence remain incomplete. |
...
| Paper datasets include CTI-MCQ, CTI-RCM, LEXam, GSM8k, and selected Global MMLU subsets (Sec. 3.2). | Approximated | `configs/paper_dataset_splits.json` pins candidate source revisions, split names, formats, and expected counts for all five datasets. Standard-library adapters cover CTIBench TSV rows plus record-level LEXam and Global MMLU MCQ conversion; GSM8k JSONL remains supported. `dataset_verification.py` records local hashes/counts where formats are readable and requires explicit parquet counts without optional dependencies. Data acquisition, license review, and exact paper-selected Global MMLU subset confirmation remain incomplete. |
...
3. Add download-free benchmark preparation/loading around
   `configs/paper_dataset_splits.json`; review licenses, add parquet-capable
   optional tooling only if needed, and confirm the exact paper-selected Global
   MMLU subset.
