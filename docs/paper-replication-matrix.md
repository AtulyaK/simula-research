...
| Downstream evaluation uses Gemma 3 4B students, Gemini 2.5 Flash teacher data, LoRA, ten seeds, and dataset-size scaling (Sec. 3.4, App. F.1). | Approximated | `downstream_evaluation.py` validates the paper-shaped plan and benchmark result records; `benchmark_evaluation.py` provides model-agnostic local CTI/GSM8k prediction loading and scoring; `run_pipeline` persists supplied results as `60_evaluation/downstream_evaluation_results.json` alongside the evaluation handoff. Training and inference backends remain unimplemented. |
