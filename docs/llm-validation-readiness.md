...
- **Fixed benchmark splits:** `configs/paper_dataset_splits.json` pins candidate source revisions, formats, split names, and full-source record counts for CTI-MCQ, CTI-RCM, LEXam, GSM8k, and Global MMLU. `configs/paper_global_mmlu_selection.json` pins the paper-described Global MMLU subset (1,436 Mathematics/Computer Science/Physics test rows per English, Korean, and Nepali config; 1.44k rounded in the paper). `dataset_adapters.py` provides standard-library CTIBench TSV and Global MMLU JSONL loading, including executable subject filtering; parquet filtering/acquisition/loading remains an explicit operator concern.
...
- **Prediction scoring command:** `py -3 -m simula_research.cli score-benchmark` loads local CTI-MCQ, CTI-RCM, GSM8k, LEXam JSONL, or selected Global MMLU JSONL records plus a task-ID prediction artifact and emits a protocol-shaped result. For LEXam, pass `--lexam-config`; for Global MMLU, pass `--global-mmlu-config` and `--selection-manifest` to apply the pinned paper subset. Pass `--local-manifest` to recheck the file path, SHA-256, and row count before scoring. This boundary does not require model packages; its output can be supplied to `run_pipeline(..., downstream_evaluation_results=...)`.
- **Remote prediction command:** `py -3 -m simula_research.cli predict-benchmark` sends each supported local task to the configured NVIDIA NIM model and writes a strict task-ID prediction artifact. This is useful for bounded real-provider smoke runs, but Kimi/NIM output is a non-paper-equivalent model result and must not be reported as Gemma 3 student evidence.
...
- **Provider-backed generation:** `SIMULA_GENERATION_BACKEND=nim` opt-in wires Kimi/NIM JSON generation into taxonomy expansion, local diversification, complexification, and critic-disagreement regeneration retries. Responses are strict JSON, stage contracts remain enforced, and deterministic providers remain the default. Generation provider failures are surfaced and recorded in the existing sanitized provider event log.
- Generation requests within each provider-backed stage share the configured `SIMULA_HTTP_MIN_INTERVAL_SECONDS` pacing gate, so best-of-N, refinement, and node-mix batches do not bypass the free-tier rate-limit guard.
