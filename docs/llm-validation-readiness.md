...
- **Prediction scoring command:** `py -3 -m simula_research.cli score-benchmark` loads local CTI-MCQ, CTI-RCM, or GSM8k records plus a task-ID prediction artifact and emits a protocol-shaped result. Pass `--local-manifest` to recheck the file path, SHA-256, and row count before scoring. This boundary does not require model packages; its output can be supplied to `run_pipeline(..., downstream_evaluation_results=...)`.
