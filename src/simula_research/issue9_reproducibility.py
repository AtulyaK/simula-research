...
ALLOWED_MIXED_COMPARABILITY_AXES: frozenset[str] = frozenset({"critic_adjudication_configuration"})
...
def _max_metric_delta(baseline_gate_report: dict[str, Any], rerun_gate_report: dict[str, Any]) -> float:
    return float(_metric_delta_summary(baseline_gate_report, rerun_gate_report)["max_metric_delta"])
...
def _evaluate_comparability_gate(milestone_review: dict[str, Any]) -> dict[str, Any]:
    ...
    for axis, axis_payload in checks.items():
        ...
        if status == "pass":
            continue
        if (
            axis in ALLOWED_MIXED_COMPARABILITY_AXES
            and status == "mixed"
            and "ablation" in details
        ):
            continue
